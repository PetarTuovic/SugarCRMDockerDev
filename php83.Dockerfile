...
RUN apt-get update && apt-get install -y wget openssl curl libcurl4-openssl-dev libffi-dev libldb-dev libldap2-dev libonig-dev libxslt-dev bash nano coreutils libc-client-dev libkrb5-dev libpng-dev libsodium-dev libgmp-dev libzip-dev libargon2-dev libxml2-dev && rm -r /var/lib/apt/lists/*
